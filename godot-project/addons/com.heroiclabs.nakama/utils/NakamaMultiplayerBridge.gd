...
func _init(p_nakama_socket: NakamaSocket) -> void:
    print_debug("init socket with " + p_nakama_socket.to_string())
...
func create_match(match_id="") -> void:
    ...
    var res
    if match_id == "":
        res = await _nakama_socket.create_match_async()
    else:
        res = await _nakama_socket.create_match_async(match_id)
    if res.is_exception():
        match_join_error.emit(res.get_exception())
        leave()
        return

    print_debug(match_id)
    _setup_match(res)
    _setup_host()
