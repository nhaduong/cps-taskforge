...
func get_player_characters_async() -> Array:
    ...
    if storage_objects.objects.size() > 0:
        var json = JSON.new()
        var result = json.parse(storage_objects.objects[0].value)
        var decoded: Array = json.data.characters
...
func create_player_character_async(color: Color, name: String) -> int:
    var availability_response: NakamaAPI.ApiRpc = await _client.rpc_async(_session, "register_character_name", name)
...
func get_last_player_character_async() -> Dictionary:
    ...
    var json = JSON.new()
    var result = json.parse(storage_objects.objects[0].value)
    var decoded: Dictionary = json.data
...
func _write_chatlogs_async(chatlogs: Array) -> int:
    # we want to only ever append content not overwrite, so first get existing chat logs
    var storage_objects: NakamaAPI.ApiStorageObjects = await _client.read_storage_objects_async(
            _session, [NakamaStorageObjectId.new("chatlogs", "chatlogs", _session.user_id)]
        )
    ...
    if storage_objects.objects.size() != 0:
        var json = JSON.new()
        var _result = json.parse(storage_objects.objects[0].value)
        var decoded: Dictionary = json.data
        _chatlogs["chatlogs"] = decoded.chatlogs
    else:
        _chatlogs["chatlogs"] = []
    _chatlogs["chatlogs"] += chatlogs
    
#    print_debug("current chat logs in storage: ", _chatlogs["chatlogs"])
        ...
        _client.write_storage_objects_async(
            ...
            [
                NakamaWriteStorageObject.new(
                    ...
                    ReadPermissions.OWNER_READ,
                    WritePermissions.OWNER_WRITE,
                    JSON.stringify({chatlogs = _chatlogs["chatlogs"]}),
                    ""
                )
            ]
        )
