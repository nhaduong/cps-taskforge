...
# API provided by Nakama in the Fish Game demo
var min_players := 1
var max_players := 10
var client_version := 'dev'

var nakama_multiplayer_bridge: NakamaMultiplayerBridge: set = _set_multiplayer_bridge # , get = _get_multiplayer_bridge

# Nakama variables:
var nakama_socket: NakamaSocket
var match_id: String: set = _set_match_id, get = get_match_id
var matchmaker_ticket: String: set = _set_matchmaker_ticket

var players: Dictionary: set = _set_readonly_variable

var _exception_handler := ExceptionHandler.new()
#var _authenticator := Authenticator.new(Online.nakama_client, _exception_handler)
var _storage_worker: StorageWorker

func _set_match_id(_id: String):
    match_id = _id
func _set_matchmaker_ticket(ticket: String):
    matchmaker_ticket = ticket
enum MatchState {
    LOBBY = 0,
    MATCHING = 1,
    CONNECTING = 2,
...
func _set_readonly_variable(_value) -> void:
    pass
func _set_multiplayer_bridge(bridge: NakamaMultiplayerBridge) -> void:
    nakama_multiplayer_bridge = bridge
func _get_multiplayer_bridge() -> NakamaMultiplayerBridge:
    if not nakama_multiplayer_bridge:
        nakama_multiplayer_bridge = NakamaMultiplayerBridge.new(nakama_socket)
    return nakama_multiplayer_bridge

func _set_nakama_socket(_nakama_socket: NakamaSocket) -> void:
    if nakama_socket == _nakama_socket:
        return
    if nakama_socket:
        nakama_socket.closed.disconnect(_on_nakama_socket_closed)


    if nakama_multiplayer_bridge:
        nakama_multiplayer_bridge.match_joined.disconnect(_on_match_joined)
#        nakama_multiplayer_bridge.match_join_error.disconnect(_on_match_join_error)
    ...
    if nakama_socket:
        nakama_socket.closed.connect(_on_nakama_socket_closed)
        nakama_socket.received_channel_message.connect(_on_NamakaSocket_received_channel_message)

        
        nakama_multiplayer_bridge = NakamaMultiplayerBridge.new(nakama_socket)
        nakama_multiplayer_bridge.match_joined.connect(_on_match_joined)
#        nakama_multiplayer_bridge.match_join_error.connect(_on_match_join_error)
...
func create_match(_nakama_socket: NakamaSocket, match_id = "") -> void:
    leave()
    _set_nakama_socket(_nakama_socket)
#    await nakama_multiplayer_bridge
    match_mode = MatchMode.CREATE

    nakama_multiplayer_bridge.create_match(match_id)
...
func _on_NamakaSocket_received_channel_message(message: NakamaAPI.ApiChannelMessage) -> void:
    ...
    emit_signal("chat_message_received", message.sender_id, content.msg, message)
...
func send_text_async(text: String) -> int:
    var error_message = "Server not connected."
    ...
    var message_response: NakamaRTAPI.ChannelMessageAck = await nakama_socket.write_chat_message_async(dynamic_chat_channel.id, data)

    var parsed_result := _exception_handler.parse_exception(message_response)
    print_debug(data, message_response, parsed_result, match_id)
    if parsed_result != OK:
        emit_signal(
            "chat_message_received", "SYSTEM", "Error code %s: %s" % [parsed_result, error_message, message_response]
        )

    # and write the text to database
    
    # Online._storage_worker._write_chatlogs_async(
    #     [
    #         {"username": message_response.username,
    #         "create_time": message_response.create_time,
    #         "message": text}
    #     ]
    #     )
    return parsed_result

var dynamic_chat_channel: NakamaRTAPI.Channel
func join_chat_async():
    var roomname = get_match_id()
    var persistence = false
    var hidden = false
    var type = NakamaSocket.ChannelType.Room
    var channel: NakamaRTAPI.Channel = await nakama_socket.join_chat_async(roomname, type, persistence, hidden)

    print("Connected to dynamic room channel: '%s'" % [channel.id])
    dynamic_chat_channel = channel
